...
namespace pboFinalProfject.View.Mahasiswa
{
    public partial class FormKuesioner : Form
    {
        private readonly ScreeningController _screeningController;
        private List<PertanyaanAssessment> _questions;
        private List<GroupBox> _questionBoxes = new List<GroupBox>();
        ...
        private void WireSidebar()
        {
            ...
            btnKonsultasi.Click += (s, e) => { new FormBuatBooking().ShowDialog(this); };
            btnProfile.Click += (s, e) => { new FormProfilMahasiswa().ShowDialog(this); };
            btnBeranda.Click += (s, e) => { Navigation.GoToDashboard(this); };
        }
        ...
        private void ShowLatestScore()
        {
            try
            {
                var latest = _screeningController.GetHasilScreeningTerakhir();
                if (latest != null)
                {
                    lblLastScore.Text = $"Skor terakhir: {latest.SkorTotal} ({latest.TingkatStres}) - {latest.TanggalAssessment:d}";
                }
            ...
            {
                lblLastScore.Text = "Gagal memuat skor terakhir.";
        ...
        private void LoadQuestions()
        {
            try
            {
                _questions = _screeningController.GetPertanyaan();
                int y = 10;
                int idx = 0;
                ...
                foreach (var q in _questions)
                {
                    ...
                    {
                        Text = "A (Ringan)",
                        Tag = new Tuple<int, char, int>(q.BobotA, 'A', q.PertanyaanId),
                    ...
                    {
                        Text = "B (Sedang)",
                        Tag = new Tuple<int, char, int>(q.BobotB, 'B', q.PertanyaanId),
                    ...
                    {
                        Text = "C (Berat)",
                        Tag = new Tuple<int, char, int>(q.BobotC, 'C', q.PertanyaanId),
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _questionBoxes.Count; i++)
            {
                ...
                if (selected == null)
                {
                    MessageBox.Show($"Silakan jawab semua pertanyaan. Pertanyaan nomor {i + 1} belum dijawab.",
                        "Validasi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                var tuple = (Tuple<int, char, int>)selected.Tag;
                answers.Add(new JawabanAssessment
                {
                    PertanyaanId = tuple.Item3,
                    Jawaban = tuple.Item2,
                    Nilai = tuple.Item1
                });
            }

            // Hitung skor menggunakan ScreeningController
            HasilAssessment hasil = _screeningController.HitungSkor(answers);

            // Konfirmasi sebelum menyimpan
            DialogResult confirm = MessageBox.Show(
                $"Skor Anda: {hasil.SkorTotal}\n" +
                $"Tingkat Stres: {hasil.TingkatStres}\n\n" +
                $"{hasil.Rekomendasi}\n\n" +
                "Apakah Anda ingin menyimpan hasil ini?",
                "Hasil Kuesioner",
            ...
            if (confirm == DialogResult.Yes)
            {
                try
                {
                    bool berhasil = _screeningController.SimpanHasilAssessment(hasil, answers);
                    ...
                    {
                        MessageBox.Show("Gagal menyimpan hasil kuesioner.", "Error",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                catch (Exception ex)
